    ...
    return function ServerDatabaseComponent(props: T) {
        ...
        const observer = (servers: ServersModel[]) => {
            ...
            if (server) {
                ...
                if (database) {
                    const defaultUrl = sanitizeUrl(LocalConfig.DefaultServerUrl);
                    const activeUrl = sanitizeUrl(server.url);
                    const serverDisplayName = activeUrl === defaultUrl ? LocalConfig.DefaultServerName : server.displayName;
                    setState({
                        database,
